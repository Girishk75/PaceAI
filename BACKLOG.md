...
| B19 | Voice override backfires — worse than system default | `native/src/services/aiCoach.ts` (`selectBestVoice`) | v2.6.8 forces a voice pick, but on the OnePlus 11R it selects a robotic voice while the (good) system-default voice — the one the Android TTS ▶ preview plays — is ignored. Confirmed 2026-08-25: preview sounds good, app is robotic. Fix: **remove the `selectBestVoice` override** and respect the system-selected voice (OS is source of truth; user picks/previews it in Android TTS settings). Keep rate/pitch/ducking. Small patch. |
...
| D14 | Robotic coach voice | v2.6.8 | initTTS() now selects the highest-quality installed offline English voice (en-IN→en-GB→en-US) instead of the default. Quality ceiling = installed voice data. |
