...
| B17 | Debug log "did not show up" on 2026-08-25 run | `native/src/services/debugLogFile.ts` | Root cause found: debug mode wasn't applied at startup (only synced when Settings opened). Fixed in v2.6.7. Close after user confirms the debug log now appears without opening Settings first. |
...
| B12 | Overpronation may over-trigger | Firmware | 40+ overpronation coach events in the 2026-07-18 run. Either genuine or `PRON_OVER_DEG=8°` is too tight. Blocked on B7 (slow-mo video validation). Note: v2.7.1 Rule 2 already suppresses overpronation cues while overpaced/Zone 4, which cuts most of the spam. |
| B20 | Coach: escalate, don't repeat (Rules 3&4) | `native/src/services/aiCoach.ts` (prompt) | Deferred half of the coaching-message rework. Rule 3: feed recent cue history + how long a condition has persisted into the LLM prompt so it stops repeating and escalates ("still too fast — walk 30s"). Rule 4: if the runner sustains an off-target pace despite alerts (a choice, e.g. 2026-09-03), acknowledge and shift to safety-only monitoring instead of nagging. |
...
| D13 | Start-button lag (blocking GPS cold-fix) | v2.7.0 | Was B18 ("Option A"). Setup screen now warms GPS via watchPositionAsync with a ACQUIRING/READY indicator; START no longer blocks on getCurrentPositionAsync — run screen/timer start instantly. |
| D14 | Robotic coach voice | v2.6.8 → reverted v2.7.1 | v2.6.8 auto-picked a voice but on the OnePlus 11R chose a robotic one over the good system default. v2.7.1 removed the override and respects the OS-selected voice (B19). |
| D15 | Coach over-triggering / spam | v2.7.1 (Rules 1&2) | 2026-09-03 run fired ~1 cue/15s. Rule 1: global 40s gap between cues. Rule 2: suppress symptom cues (impact/fatigue/cadence/strike/pronation) while a pace/HR root cause is active. Detection was correct (Garmin: 6:29/km vs 7:30 target) — only delivery changed. Escalate-don't-repeat (Rules 3&4) still to do — see B20. |
