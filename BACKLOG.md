...
| ID | Item | File | Notes |
|---|---|---|---|
| B16 | Coach goes silent with no internet mid-run (offline fallback) | `native/src/services/aiCoach.ts` | **Highest-value open item.** Every cue is a live Claude API call; no signal = no coaching. Confirmed 3 runs (treadmill died 32s, forest 14s, 2026-08-25 14s). Fix: rule-based/templated offline cues (low_cad, high_hr, pace, km splits) that speak with no network, with the LLM upgrading them when online. Ties to phone-free research (edge LLM not viable; rules are the offline path). |
