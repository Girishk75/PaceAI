...
| B18 | GPS-ready before Start (remove start lag) — "Option A" | `native/src/screens/SetupScreen.tsx`, `native/src/hooks/useGPS.ts` | Approved 2026-08-25, queued separate from the voice change. Tapping Start currently blocks on `prewarmGPS()`'s `getCurrentPositionAsync` (2–10s cold fix) before the run screen/timer start. Plan: warm GPS while the Setup screen is open, show a "GPS: acquiring…/✓ ready" indicator, and start the run instantly (no blocking fix). Note: distance/pace still need a few seconds for first lock (physics); this removes the perceived freeze. |
